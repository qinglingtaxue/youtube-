...
as_a: 个人创作者
i_want: 自动发现数据中的高价值模式
so_that: 找到被验证但传播不足的创作规律

acceptance_criteria:
  - AC1: 支持 5 个分析维度（变量、时间、空间、频道、用户）
  - AC2: 每个模式包含有趣度评分（1-5 分）
  - AC3: 每个模式包含置信度百分比
  - AC4: 每个模式包含数据源链接
  - AC5: 模式按有趣度降序排列
...
as_a: 个人创作者
i_want: 了解标题长度、视频时长等变量与播放量的关系
so_that: 优化自己视频的基础参数

acceptance_criteria:
  - AC1: 分析标题长度与播放量的关系
  - AC2: 分析视频时长与播放量的关系
  - AC3: 发现最优参数区间（如标题 20-40 字，时长 10-30 分钟）
  - AC4: 生成可视化图表
  - AC5: 提供具体的参数建议
...
as_a: 个人创作者
i_want: 了解不同语言市场的表现差异
so_that: 发现跨语言套利机会

acceptance_criteria:
  - AC1: 对比中文 vs 英语市场的平均播放量
  - AC2: 识别中文火爆但英语空白的话题
  - AC3: 识别英语火爆但中文空白的话题
  - AC4: 估算 CPM 收益差异
  - AC5: 提供跨语言创作建议
...
as_a: 个人创作者
i_want: 通过可视化界面浏览 42 个模式
so_that: 按需探索感兴趣的模式

acceptance_criteria:
  - AC1: 6 大板块卡片分类展示（全局认识、套利挖掘、选题决策、内容创作、发布策略、频道运营）
  - AC2: 支持关键词搜索
  - AC3: 支持按有趣度/维度筛选
  - AC4: 点击进入模式详情页
  - AC5: 响应式设计，支持移动端
...
priority: P0 (MVP)
effort: S (较小)
dependencies: [US-PAT001, US-PAT002, US-PAT003]
```
...
| **US-PAT001** | **多维度模式发现** | **模式分析** | **M** |
| **US-PAT002** | **变量分布分析** | **模式分析** | **S** |
| **US-PAT003** | **跨语言市场分析** | **模式分析** | **M** |
| **US-PAT004** | **学习路径导航** | **模式分析** | **M** |
| **US-PAT005** | **行动指南生成** | **模式分析** | **S** |
| US-P001 | 脚本生成 | 策划 | M |
| US-P002 | SEO 优化 | 策划 | S |
| US-P003 | 视频规约生成 | 策划 | S |
| US-M002 | 字幕处理 | 制作 | M |
...
| 1.4 | 2026-01-24 | 新增模式分析阶段用户故事(US-PAT001~US-PAT005)，总计 34 个用户故事 |
| 1.5 | 2026-01-24 | 更新有趣度公式表述，区分原理层和求解层 |
| 1.6 | 2026-01-26 | 反映 COG 模型中图表实体和模式系统的改进，TrendingTracker 和 PatternDetail 定义清晰化 |
