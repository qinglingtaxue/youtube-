...
### 2.9 PatternAnalysis (模式分析)

```yaml
entity: PatternAnalysis
description: 多维度模式分析结果
...
fields:
  ...
  - name: pattern_id
    ...
    nullable: false
    unique: true
    description: 模式编号 (如 P001, P023, P042)
...
indexes:
  - name: idx_pattern_dimension
    fields: [dimension]
  - name: idx_pattern_difficulty
    fields: [difficulty]
```

### 2.10 LearningPath (学习路径)
...
fields:
      ...
      [
        {
          "block_id": "B001",
          "title": "理解有趣度公式",
          "type": "concept",  // concept, pattern, action
          "content": "内容...",
          "patterns": ["P001", "P002"],
          "next_blocks": ["B002"]
        }
  ...
  - name: patterns
    type: JSON
    description: 包含的模式 ID 列表

  - name: estimated_duration
    type: VARCHAR(50)
    description: 预计学习时长 (如 "2-3 周")
...
indexes:
  - name: idx_learning_path_tier
    fields: [tier]
```

### 2.11 TrendSnapshot (趋势快照)
...
fields:
  ...
  - name: dimension
    type: VARCHAR(20)
    nullable: false
    enum: ["topic", "channel", "duration", "timing", "cross_language"]
    description: 趋势维度

  - name: metric_name
    type: VARCHAR(50)
    nullable: false
    description: 指标名称 (如 "rising_topics", "duration_distribution")
  ...
  - name: metric_value
    ...
    description: |
      ...
      {
        "rising_topics": [
          {"keyword": "AI 养生", "trend_ratio": 2.5, "recent_count": 50}
        ],
        "falling_topics": [
          {"keyword": "传统养生", "trend_ratio": 0.3}
        ]
      }
...
indexes:
  - name: idx_trend_date_dimension
    fields: [snapshot_date, dimension]
  - name: idx_trend_metric
    fields: [metric_name]

unique_constraints:
  - name: uq_trend_date_dimension_metric
    fields: [snapshot_date, dimension, metric_name]
```

### 2.12 PatternReport (模式报告)

```yaml
entity: PatternReport
description: 模式分析报告
...
fields:
  ...
  - name: report_type
    type: VARCHAR(20)
    nullable: false
    enum: ["full", "dimension", "incremental"]
    description: 报告类型
  ...
  - name: dimension
    ...
    nullable: true
    enum: ["variable", "temporal", "spatial", "channel", "user"]
    description: 分析维度 (dimension 类型报告必填)
  ...
  - name: data_summary
    ...
    description: |
      数据概要:
      {
        "total_videos": 2340,
        "languages": ["zh", "en", "ja", ...],
        "date_range": {"start": "2025-01-01", "end": "2026-01-24"},
        "patterns_discovered": 42
      }

  - name: variable_distribution
    type: JSON
    description: |
      变量分布分析结果:
      {
        "view_count": {"mean": 50000, "median": 8000, "std": 150000},
        "duration": {"mean": 600, "median": 480},
        ...
      }

  - name: temporal_analysis
    type: JSON
    description: 时间维度分析结果

  - name: spatial_analysis
    type: JSON
    description: 空间维度分析结果

  - name: channel_analysis
    type: JSON
    description: 频道维度分析结果

  - name: user_analysis
    type: JSON
    description: 用户维度分析结果
...
indexes:
  - name: idx_pattern_report_date
    fields: [report_date]
  - name: idx_pattern_report_type
    fields: [report_type]
```
...
CREATE TABLE IF NOT EXISTS pattern_analyses (
    ...
    cross_language_ratio REAL,
    examples TEXT,      -- JSON
    action_guide TEXT,  -- JSON
    related_patterns TEXT,  -- JSON
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    updated_at DATETIME DEFAULT CURRENT_TIMESTAMP
);

CREATE INDEX idx_pattern_dimension ON pattern_analyses(dimension);
CREATE INDEX idx_pattern_difficulty ON pattern_analyses(difficulty);
...
CREATE TABLE IF NOT EXISTS learning_paths (
    ...
    title TEXT NOT NULL,
    description TEXT,
    tier TEXT CHECK(tier IN ('beginner', 'mid_tier', 'top_tier')),
    goal TEXT,
    blocks TEXT,            -- JSON
    patterns TEXT,          -- JSON
...
CREATE INDEX idx_learning_path_tier ON learning_paths(tier);
...
CREATE TABLE IF NOT EXISTS trend_snapshots (
    ...
    dimension TEXT NOT NULL CHECK(dimension IN
        ('topic', 'channel', 'duration', 'timing', 'cross_language')),
    metric_name TEXT NOT NULL,
    metric_value TEXT,          -- JSON
    baseline_comparison TEXT,   -- JSON
    insights TEXT,
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    UNIQUE(snapshot_date, dimension, metric_name)
);

CREATE INDEX idx_trend_date_dimension ON trend_snapshots(snapshot_date, dimension);
CREATE INDEX idx_trend_metric ON trend_snapshots(metric_name);

-- 模式报告表
CREATE TABLE IF NOT EXISTS pattern_reports (
    report_id TEXT PRIMARY KEY,
    report_date DATE NOT NULL,
    report_type TEXT NOT NULL CHECK(report_type IN ('full', 'dimension', 'incremental')),
    dimension TEXT CHECK(dimension IN
        ('variable', 'temporal', 'spatial', 'channel', 'user')),
    data_summary TEXT,              -- JSON
    variable_distribution TEXT,     -- JSON
    temporal_analysis TEXT,         -- JSON
    spatial_analysis TEXT,          -- JSON
    channel_analysis TEXT,          -- JSON
    user_analysis TEXT,             -- JSON
...
CREATE INDEX idx_pattern_report_date ON pattern_reports(report_date);
CREATE INDEX idx_pattern_report_type ON pattern_reports(report_type);
...
| 1.3 | 2026-01-24 | 新增模式分析实体 (PatternAnalysis)：5维度模式存储、42模式详情 |
| 1.4 | 2026-01-24 | 新增学习路径实体 (LearningPath)：博主层级学习区块、模式关联 |
| 1.5 | 2026-01-24 | 新增趋势快照实体 (TrendSnapshot)：趋势监控、基线对比 |
| 1.6 | 2026-01-24 | 新增模式报告实体 (PatternReport)：全维度分析报告、行动建议 |
| 1.7 | 2026-01-26 | 重命名图表实体 (ScatterChart → ContentQuadrantChart, BarChart → DurationDistributionChart)，补充 TrendingTracker 和 PatternDetail 定义 |
