...
>
> 详细约束见：real.md Appendix C

<UserCognitionModel>
- 唯一编码：user_cognition (singleton)
- 用途：定义用户能理解和不能理解的概念边界，指导 UI 设计
...
| 评论数 | 越多讨论越热 | 直接显示数字 |
| 订阅数 | 频道大小 | 直接显示数字 |
| 时长 | 短/中/长 | 使用 YouTube 原生分类 |
| 发布时间 | 新/旧 | "X天前" 格式 |
| 日均播放 | 每天多少人看 | 显示数字 + 简短说明 |
...
必须使用 YouTube 原生分类：
- 时长：<4min / 4-20min / >20min
- 时间范围：1h / 24h / 7d / 30d / 1yr
...
<UserSearchHistory>
- 唯一编码：search_history (per user)
- 存储位置：localStorage
- 核心属性：
  - keywords：搜索关键词列表（最近10个）
  - last_filters：上次使用的筛选条件
  - last_sort：上次使用的排序方式
  - collapsed_states：各面板的折叠状态

**显示位置**：搜索框正下方，一键点击可重新搜索
</UserSearchHistory>

<SortingControl>
- 唯一编码：sorting_control (per list)
- 用途：列表排序的三维控制组件
- 核心属性：
  - time_range：时间范围（24h/7d/30d/1yr/全部）
  - sort_field：排序字段（播放量/点赞数/评论数/日均播放/时长/发布时间）
  - sort_direction：排序方向（高→低/低→高 或 新→旧/旧→新）
  - current_rule_text：当前规则的自然语言描述
...
<FilterPanel>
- 唯一编码：filter_panel (per page)
- 用途：筛选条件面板
- 核心属性：
  - is_collapsed：是否折叠（默认 true）
  - selected_count：已选条件数量
  - filters：筛选条件对象
    - time_range：时间范围
    - duration：时长（使用 YouTube 原生分类）
    - channel_size：频道规模
    - min_views：最低播放量
    - content_tags：内容标签（多选）

**折叠状态显示**：
```
[▶ 展开筛选条件]              ← 默认状态
[▼ 收起筛选条件] 已选3项      ← 展开后，显示已选数量
```
