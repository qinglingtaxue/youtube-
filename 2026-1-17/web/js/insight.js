        ...
        // 全局结论存储 - 各板块分析完成后将结论存入此处
        // 格式：{ patternId: '模式ID', patternName: '模式名称', dataSource: '数据来源', conclusion: '结论' }
        const tabConclusions = {
            tab1: { name: '全局认知', icon: '🌍', items: [] },
            tab2: { name: '套利挖掘', icon: '💰', items: [] },
        ...
        // 注册模式结论到对应板块
        // @param tabId - 板块ID
        // @param patternId - 模式ID（如 "4", "12", "23"）
        // @param patternName - 模式名称（如 "内容类型天花板"）
        // @param dataSource - 数据来源（如 "内容类型播放量分布"）
        // @param conclusion - 具体结论文字
        // @param examples - 可选，案例数据数组 [{ title, channel, views, url, ... }]
        function registerPatternConclusion(tabId, patternId, patternName, dataSource, conclusion, examples = null) {
            if (tabConclusions[tabId]) {
                tabConclusions[tabId].items.push({
                    patternId,
                    patternName,
                    dataSource,
                    conclusion,
                ...
                console.log(`[模式结论] ${tabConclusions[tabId].name} - 模式${patternId}：${patternName}${examples ? ` (${examples.length}个案例)` : ''}`);
            }
        }

        // 旧版兼容（简化版注册）
        function registerConclusion(tabId, dataSource, conclusion) {
            if (tabConclusions[tabId]) {
                tabConclusions[tabId].items.push({
                    patternId: '',
                    patternName: '',
                ...
                if (count === 0) {
                    ...
                    items.forEach((item, idx) => {
                        const patternLabel = item.patternId && item.patternName
                            ...
                            : '';
                        conclusionsHtml += `
                            <div class="pattern-conclusion-item">
                                <span class="conclusion-number">${idx + 1}.</span>
                                <span class="conclusion-text">
                                    基于「${item.dataSource}」的数据得出${patternLabel}结论：${item.conclusion}
                                </span>
                            </div>
                        `;
                    });
                ...
                html += `
                    <div class="tab-review-card">
                        <div class="tab-review-header">
                            ...
                            <span class="tab-review-badge">${count} 条模式结论</span>
                        </div>
        ...
        function renderSynthesis() {
            ...
            if (p4 || p23 || p13) {
                // 提取关键数据点
                const p4Text = p4 ? p4.conclusion.split('。')[0] : '';
                const p23Text = p23 ? p23.conclusion.split('。')[0] : '';
                const p13Text = p13 ? p13.conclusion.split('。')[0] : '';

                html += `<div class="cross-card">
                    <div class="cross-card-title">📌 最佳赛道判断</div>
                    <div class="cross-card-sources">
                        ${p4 ? `<div class="source-row"><span class="source-tag">模式4</span>${p4Text}</div>` : ''}
                        ${p23 ? `<div class="source-row"><span class="source-tag">模式23</span>${p23Text}</div>` : ''}
                        ${p13 ? `<div class="source-row"><span class="source-tag">模式13</span>${p13Text}</div>` : ''}
                    </div>
                    <div class="cross-card-conclusion">
                        ∴ <strong>功法养生</strong>：天花板21.5万(模式4) + 垄断度相对低(模式23) + 18年长青(模式13) → 三重验证的最佳赛道
                    </div>
                </div>`;
            }
            ...
            if (p12 || p2) {
                html += `<div class="cross-card">
                    ...
                    <div class="cross-card-conclusion">
                        ∴ <strong>1-5万订阅</strong>爆发率最高(模式12) + 大频道效率边际递减(模式2) → 新人起步无劣势，内容质量比粉丝数更重要
                    </div>
                </div>`;
            }
            ...
            if (p3 || p7 || p10) {
                html += `<div class="cross-card">
                    ...
                    <div class="cross-card-conclusion">
                        ∴ 公式：<strong>10-30分钟</strong>(15.9倍) + <strong>含数字</strong>(+100%) + <strong>陈述句</strong>(+63%) = 综合提升潜力超300%
                    </div>
                </div>`;
            }

            // 4. 地域套利 - 引用CPM差异
            const p20 = getPattern('tab2', '20');
            const p24 = getPattern('tab2', '24');
            if (p20 || p24) {
                html += `<div class="cross-card">
                    <div class="cross-card-title">📌 地域套利机会</div>
                    <div class="cross-card-sources">
                        ${p20 ? `<div class="source-row"><span class="source-tag">模式20</span>${p20.conclusion.split('。')[0]}</div>` : ''}
                        ${p24 ? `<div class="source-row"><span class="source-tag">模式24</span>${p24.conclusion.split('。')[0]}</div>` : ''}
                    </div>
                    <div class="cross-card-conclusion">
                        ∴ 同样10万播放：<strong>美国$1,200</strong> vs <strong>台湾$350</strong>(差3.4倍) → 优先做英语Tai Chi内容，瞄准美国市场
                    </div>
                </div>`;
            }
        ...
        function renderActionChecklist() {
            ...
            Object.entries(tabConclusions).forEach(([tabId, tab]) => {
                (tab.items || []).forEach(item => {
                    allItems.push({ ...item, source: tab.name, tabId });
                });
            });

            if (allItems.length === 0) {
                checklistEl.innerHTML = '<div class="action-loading">请先查看其他板块的分析，行动建议将自动生成</div>';
                return;
            }

            // 取最重要的几条作为行动项
            checklistEl.innerHTML = allItems.slice(0, 5).map(item => `
                <div class="action-item">
                    <div class="action-checkbox"></div>
                    <div class="action-text">
                        ${item.conclusion}
                        <div class="action-source">来源：<span onclick="jumpToTab('${item.tabId}')">${item.source}</span></div>
                    </div>
                </div>
            `).join('');
        }
        ...
        // 从结论中提取行动建议（保留兼容）
        function extractActionsFromItems(allItems) {
            // 直接返回结论作为行动建议
        ...
        // 旧版函数保留兼容
        function extractActionsFromConclusions(allConclusions) {
            const actions = [];

            // 根据结论生成对应行动
            allConclusions.forEach(item => {
                const text = item.text || item.conclusion || '';
                // 简单示例：将结论转化为行动建议
                if (text.includes('时长') || text.includes('分钟')) {
                    actions.push({
                        text: `控制视频时长：${text}`,
                        source: item.source,
                        tabId: getTabIdByName(item.source)
                    });
                } else if (text.includes('标题') || text.includes('公式')) {
                    actions.push({
                        text: `优化标题：${text}`,
                        source: item.source,
                        tabId: getTabIdByName(item.source)
                    });
                } else if (text.includes('发布') || text.includes('周')) {
                    actions.push({
                        text: `发布计划：${item.text}`,
                        source: item.source,
                        tabId: getTabIdByName(item.source)
            ...
            // 如果没有提取到具体行动，给出默认建议
            if (actions.length === 0 && allConclusions.length > 0) {
                actions.push({
                    text: '查看各板块详细分析，制定具体创作计划',
                    source: '综合建议',
                    tabId: 'tab1'
                });
            }

            return actions.slice(0, 5); // 最多显示5条
        }
        ...
        function updatePatternsWithData(data) {
            ...
            if (data.channels && data.channels.length > 0) {
                ...
                try {
                    renderPattern31(data.channels);
                    console.log('✓ 模式31(高手画像)渲染完成');
                } catch(e) { console.error('✗ 模式31错误:', e); }
            } else {
                console.warn('没有频道数据！');
            }
        ...
        function renderBar(canvasId, labels, values, config = {}) {
            console.log(`renderBar(${canvasId}) - labels:`, labels, 'values:', values);
            const ctx = document.getElementById(canvasId);
            if (!ctx) {
                console.error(`renderBar: canvas '${canvasId}' 未找到!`);
                return null;
            }
            console.log(`renderBar(${canvasId}) - canvas 找到，开始渲染`);
        ...
        /**
         * 通用热力图渲染器（使用 Chart.js matrix 插件或简化版本）
         * 这里用散点图模拟热力图效果
         * @param {string} canvasId - canvas元素ID
         ...
         * 通用水平条形图（排行榜专用）
         * @param {string} canvasId - canvas元素ID
         * @param {Array} items - 数据项 [{label, value, color?}, ...]
         * @param {Object} config - 配置
         */
        function renderRankingBar(canvasId, items, config = {}) {
            const ctx = document.getElementById(canvasId);
            if (!ctx) return null;
            ...
            const colors = items.map((_, i) => {
                if (i === 0) return 'rgba(239, 68, 68, 0.8)';  // 第1名 红色
                if (i === 1) return 'rgba(249, 115, 22, 0.8)'; // 第2名 橙色
                if (i === 2) return 'rgba(234, 179, 8, 0.8)';  // 第3名 金色
                return 'rgba(6, 182, 212, 0.6)';               // 其他 青色
            });
                ...
                options: {
                    ...
                    scales: {
                        x: {
                            ...
                            title: config.xLabel ? { display: true, text: config.xLabel, color: '#94a3b8' } : undefined
                        },
                        y: {
                            grid: { display: false },
                            ticks: { color: '#e2e8f0' }
        ...
        /**
         * 通用词云/星云图
         * @param {string} containerId - 容器div的ID
         * @param {Array} words - 词语数据 [{text, weight, color?}, ...]
         * @param {Object} config - 配置
         */
        function renderWordCloud(containerId, words, config = {}) {
            ...
            // 创建词云HTML
            const cloudHTML = sorted.slice(0, config.maxWords || 50).map((word, i) => {
                // 计算字体大小（12-48px）
                const normalized = (word.weight - minWeight) / (maxWeight - minWeight + 0.01);
                ...
                if (word.color) {
                    ...
                    color = 'rgba(239, 68, 68, 0.9)';  // Top 3 红色
                } else if (i < 10) {
                    color = 'rgba(249, 115, 22, 0.8)'; // Top 10 橙色
                } else if (normalized > 0.5) {
                    color = 'rgba(6, 182, 212, 0.8)';  // 高权重 青色
                } else {
                    color = 'rgba(148, 163, 184, 0.7)'; // 低权重 灰色
                }
                ...
                return `<span class="word-cloud-item" style="
                    ...
                    cursor: pointer;
                    transition: all 0.2s;
                " data-weight="${word.weight}" title="${word.text}: ${config.valueFormatter ? config.valueFormatter(word.weight) : word.weight}">${word.text}</span>`;
            }).join('');
            ...
            // 添加悬停效果
            container.querySelectorAll('.word-cloud-item').forEach(item => {
         ...
         * 通用树图/矩形热力图
         * @param {string} containerId - 容器div的ID
         * @param {Array} data - 数据 [{label, value, children?}, ...]
         * @param {Object} config - 配置
         */
        function renderTreemap(containerId, data, config = {}) {
            const container = document.getElementById(containerId);
            ...
            const containerHeight = config.height || 280;

            // 简化的Treemap布局算法
            let x = 0, y = 0;
            let rowHeight = containerHeight;
            ...
            while (remaining.length > 0) {
                ...
                if (rowWidth > rowHeight) {
                    w = (item.value / total) * containerWidth;
                    h = containerHeight;
                } else {
                    w = containerWidth;
                    h = (item.value / total) * containerHeight;
                }
                ...
                rects.push({
                    ...item,
                    x: x % containerWidth,
                    y: Math.floor(x / containerWidth) * 40,
                    w: Math.min(w, containerWidth - (x % containerWidth)),
                    h: Math.min(h, 40),
                    ratio
                });
            ...
            const rectsHTML = data.slice(0, 20).map((item, i) => {
                const ratio = item.value / total;
                const width = Math.max(ratio * containerWidth * 2, 60);
                const height = 35;
                const color = item.color || colors[i % colors.length];

                return `<div class="treemap-item" style="
                    display: inline-flex;
                    align-items: center;
                    justify-content: center;
                    width: ${width}px;
                    ...
                    white-space: nowrap;
                " title="${item.label}: ${config.valueFormatter ? config.valueFormatter(item.value) : item.value}">
         ...
         * 漏斗图
         * @param {string} containerId - 容器div的ID
         * @param {Array} stages - 阶段数据 [{label, value}, ...]
         * @param {Object} config - 配置
         */
        function renderFunnel(containerId, stages, config = {}) {
            const container = document.getElementById(containerId);
            ...
            const funnelHTML = stages.map((stage, i) => {
                ...
                return `
                    ...
                    ">
                        <div style="
                            ...
                            background: ${stage.color || colors[i % colors.length]};
                            border-radius: 4px;
                            display: flex;
                            align-items: center;
                            justify-content: center;
                            ...
                            transition: all 0.3s;
                            cursor: pointer;
                        " title="${stage.label}: ${config.valueFormatter ? config.valueFormatter(stage.value) : stage.value}">
                            <span>${stage.label}</span>
                            <span style="margin-left: 8px; opacity: 0.8;">${config.valueFormatter ? config.valueFormatter(stage.value) : formatNumber(stage.value)}</span>
                        </div>
                        ${i > 0 ? `<span style="font-size: 11px; color: #94a3b8;">↓${conversionRate}%</span>` : ''}
                    </div>
                `;
            }).join('');

            container.innerHTML = `
                <div style="padding: 10px;">${funnelHTML}</div>
            `;
        }
         ...
         * 桑基图（简化版流向图）
         * @param {string} containerId - 容器div的ID
         * @param {Array} flows - 流向数据 [{from, to, value}, ...]
         * @param {Object} config - 配置
         */
        function renderSankey(containerId, flows, config = {}) {
            const container = document.getElementById(containerId);
            if (!container || flows.length === 0) return;
            ...
            const nodes = new Map();
            flows.forEach(f => {
                if (!nodes.has(f.from)) nodes.set(f.from, { name: f.from, inValue: 0, outValue: 0 });
                if (!nodes.has(f.to)) nodes.set(f.to, { name: f.to, inValue: 0, outValue: 0 });
                nodes.get(f.from).outValue += f.value;
                nodes.get(f.to).inValue += f.value;
            });

            // 分层
            const leftNodes = [...nodes.values()].filter(n => n.inValue === 0 || n.outValue > n.inValue);
            const rightNodes = [...nodes.values()].filter(n => n.outValue === 0 || n.inValue > n.outValue);

            const maxValue = Math.max(...flows.map(f => f.value));

            // 渲染节点和连线
            const flowsHTML = flows.slice(0, 15).map((flow, i) => {
                const thickness = Math.max((flow.value / maxValue) * 40, 8);
                const opacity = 0.3 + (flow.value / maxValue) * 0.5;

                return `
                    <div class="sankey-flow" style="
                        display: flex;
                        align-items: center;
                        margin: 6px 0;
                        font-size: 12px;
                    ">
                        ...
                        <div style="
                            flex: 1;
                            height: ${thickness}px;
                            background: linear-gradient(90deg, rgba(6, 182, 212, ${opacity}), rgba(139, 92, 246, ${opacity}));
                            border-radius: 4px;
                            display: flex;
                            align-items: center;
                            justify-content: center;
                            color: white;
                            font-size: 10px;
                        ">${config.valueFormatter ? config.valueFormatter(flow.value) : formatNumber(flow.value)}</div>
                        <span style="
                            min-width: 80px;
                            padding-left: 8px;
                            color: #e2e8f0;
            ...
            container.innerHTML = `
                <div style="padding: 12px;">${flowsHTML}</div>
            `;
        }
         ...
         * @param {Object} config - 配置
         */
        function renderGauge(canvasId, value, max, config = {}) {
            const ctx = document.getElementById(canvasId);
            if (!ctx) return null;
            ...
            // 根据百分比选择颜色
            let color;
            if (percentage >= 80) color = 'rgba(34, 197, 94, 0.8)';      // 绿色
            else if (percentage >= 60) color = 'rgba(6, 182, 212, 0.8)'; // 青色
            else if (percentage >= 40) color = 'rgba(234, 179, 8, 0.8)'; // 黄色
            else color = 'rgba(239, 68, 68, 0.8)';                        // 红色
                ...
                plugins: [{
                    ...
                    afterDraw: (chart) => {
                        ...
                        if (config.label) {
                            ...
                            ctx.fillText(config.label, centerX, centerY + 25);
                        }
                        ctx.restore();
                    }
                }]
        ...
        function renderPattern4(videos) {
            ...
            const healthKeywords = {
                '功法养生': ['八段锦', '太极', '太極', '气功', '氣功', '站桩', '五禽戏', '易筋经', '瑜伽', '冥想', '打坐', '呼吸法', '导引', '平甩功', '保健操', '养生操', '養生操', '甩手', '拉筋', '伸展'],
                '食疗养生': ['食疗', '食療', '食补', '药膳', '藥膳', '养生茶', '養生茶', '养生汤', '豆浆', '豆漿', '枸杞', '黑豆', '生姜', '蜂蜜', '红枣', '食谱', '食譜', '吃什么', '吃什麼', '补气', '补血', '滋阴', '壮阳', '鸡汤', '雞湯', '炖汤'],
                '穴位经络': ['穴位', '按摩', '艾灸', '刮痧', '经络', '經絡', '推拿', '拍打', '敲打', '疏通', '足三里', '合谷', '涌泉', '龙筋', 'spa', 'massage'],
                '器官保健': ['眼睛', '護眼', '肝', '养肝', '養肝', '肾', '腎', '补肾', '補腎', '心脏', '心臟', '肺', '脾', '胃', '血管', '血栓', '胆固醇', '膽固醇', '三酸甘油', '脊椎', '頸椎', '颈椎'],
            ...
            console.log('模式4 - 内容类型统计:', chartData);

            // 检查canvas是否存在
            const canvas = document.getElementById('contentTypeChart');
            console.log('模式4 - canvas元素:', canvas ? '找到' : '未找到');

            if (chartData.length > 0) {
                // 1. 更新左侧表格
                const tableBody = document.getElementById('contentTypeTableBody');
                if (tableBody) {
                    const getCeilingLevel = (avgViews, maxViews) => {
                        if (maxViews > 1000000) return { text: '极高', class: 'high-value' };
                        if (maxViews > 500000) return { text: '高', class: 'high-value' };
                        if (maxViews > 100000) return { text: '中', class: '' };
                        if (maxViews > 50000) return { text: '低', class: 'low-value' };
                        return { text: '较低', class: 'low-value' };
                    };

                    tableBody.innerHTML = chartData.slice(0, 6).map((d, i) => {
                        const ceiling = getCeilingLevel(d.value, d.max);
                        const highlightClass = i === 0 ? 'highlight-value' : '';
                        return `<tr>
                            <td>${d.label}</td>
                            <td>${d.count}</td>
                            <td class="${highlightClass}">${formatNumber(d.value)}</td>
                            <td>${formatNumber(d.max)}</td>
                            <td class="${ceiling.class}">${ceiling.text}</td>
                        </tr>`;
                    }).join('');
                }

                // 2. 渲染图表
                renderBar('contentTypeChart',
                    chartData.map(d => d.label),
                    chartData.map(d => d.value),
                    { xLabel: '内容类型', yLabel: '平均播放量' }
                );

                // 3. 生成洞察
                const best = chartData[0];
                const worst = chartData[chartData.length - 1];
                const ratio = (best.value / worst.value).toFixed(1);

                updateInsight('contentTypeInsight',
                    `<strong>${best.label}</strong>类内容平均播放最高（<span class="highlight">${formatNumber(best.value)}</span>），是${worst.label}的<strong>${ratio}倍</strong>。天花板最高达<strong>${formatNumber(best.max)}</strong>。`,
                    `建议优先制作${best.label}类内容，成功概率更高。`
                );

                // 4. 更新结论区域
                const conclusionText = document.querySelector('#tab1-sub1 .pattern-conclusion-text');
                if (conclusionText) {
                    conclusionText.innerHTML = `${best.label}类内容平均播放量最高（${formatNumber(best.value)}），是${worst.label}的<strong>${ratio}倍</strong>。内容类型选择直接决定了播放量天花板，建议优先选择${best.label}类高天花板赛道。`;
                }

                // 5. 更新行动建议
                const actionsDiv = document.querySelector('#tab1-sub1 .pattern-actions');
                if (actionsDiv && chartData.length >= 2) {
                    actionsDiv.innerHTML = `
                        <span class="pattern-action do">✅ 优先做${chartData[0].label}</span>
                        <span class="pattern-action do">✅ ${chartData[1].label}也不错</span>
                        <span class="pattern-action avoid">⚠️ 避免${chartData[chartData.length - 1].label}</span>
                    `;
                }
                ...
                if (sampleEl) {
                    sampleEl.textContent = `N = ${videos.length}`;
                }

                // ========== 同步渲染到 tab3-sub3 (选题决策-内容类型天花板) ==========
                // 表格
                const tableBody2 = document.getElementById('contentTypeTableBody2');
                if (tableBody2) {
                    const getCeilingLevel = (avgViews, maxViews) => {
                        if (maxViews > 1000000) return { text: '极高', class: 'high-value' };
                        if (maxViews > 500000) return { text: '高', class: 'high-value' };
                    ...
                    tableBody2.innerHTML = chartData.slice(0, 6).map((d, i) => {
                        const ceiling = getCeilingLevel(d.value, d.max);
                        const highlightClass = i === 0 ? 'highlight-value' : '';
                        return `<tr>
                            <td>${d.label}</td>
                ...
                // 条形图
                const barChart2 = document.getElementById('contentTypeBarChart2');
                if (barChart2) {
                    const maxValue = chartData[0]?.value || 1;
                    barChart2.innerHTML = chartData.slice(0, 5).map(d => {
                        const pct = Math.round((d.value / maxValue) * 100);
                        const barClass = pct === 100 ? 'success' : (pct < 15 ? 'danger' : (pct < 30 ? 'warning' : ''));
                ...
                // 结论
                const conclusion2 = document.getElementById('contentTypeConclusion2');
                if (conclusion2) {
                    conclusion2.innerHTML = `${best.label}类播放量是${worst.label}类的 <strong>${ratio}倍</strong>，内容类型选择是最重要的战略决策`;
                }

                // 行动建议
                const actions2 = document.getElementById('contentTypeActions2');
                if (actions2 && chartData.length >= 2) {
                    actions2.innerHTML = `
                        <div class="action-item success">
                            <span class="action-icon">✅</span>
                            <span>优先做：${chartData[0].label}</span>
                        </div>
                        <div class="action-item">
                            <span class="action-icon">📊</span>
                            <span>稳定选择：${chartData[1]?.label || ''}、${chartData[2]?.label || ''}</span>
                        </div>
                        <div class="action-item danger">
                            <span class="action-icon">❌</span>
                            <span>避免做：${chartData[chartData.length - 1].label}（天花板较低）</span>
                        </div>
                    `;
                }

                // 收集每种内容类型的Top视频作为案例
                const topExamples = chartData.slice(0, 3).map(d => {
                    const typeVideos = typeStats[d.label]?.videos || [];
                    const topVideo = [...typeVideos].sort((a, b) => (b.view_count || 0) - (a.view_count || 0))[0];
                    return topVideo ? {
                        type: d.label,
                        title: topVideo.title,
                        channel: topVideo.channel_name || topVideo.channel,
                        views: topVideo.view_count,
                        url: topVideo.youtube_id ? `https://youtube.com/watch?v=${topVideo.youtube_id}` : null
                    } : null;
                }).filter(Boolean);
                ...
                registerPatternConclusion('tab1', '4', '内容类型天花板',
                    '内容类型播放量分布',
                    `${best.label}类内容平均播放量最高（${formatNumber(best.value)}），是${worst.label}的${ratio}倍。内容类型选择直接决定了播放量天花板，建议优先选择${best.label}类高天花板赛道。`,
                    topExamples
                );

                // Tab3 选题决策视角的模式4
                registerPatternConclusion('tab3', '4', '内容类型天花板',
                    '内容类型天花板',
                    `选题策略：优先做${best.label}（均播${formatNumber(best.value)}），避免${worst.label}（天花板低${ratio}倍）。${chartData[1]?.label || ''}、${chartData[2]?.label || ''}可作为稳定补充选择。`,
                    topExamples
                );
            } else {
                console.warn('模式4 - 没有足够的数据（每种类型需要>=5个视频）');
            }
        }

        /**
         * 模式23：话题垄断度分析
         * 计算头部频道的市场集中度，并分析各话题的垄断程度
         */
        function renderPattern23(channels, videos) {
            if (!channels || channels.length === 0) return;

            const sorted = [...channels].sort((a, b) => b.total_views - a.total_views);
            const totalViews = sorted.reduce((sum, c) => sum + c.total_views, 0);

            // 计算各梯队份额
            const top1Share = sorted[0] ? (sorted[0].total_views / totalViews * 100) : 0;
            const top3Views = sorted.slice(0, 3).reduce((sum, c) => sum + c.total_views, 0);
            const top3Share = (top3Views / totalViews * 100);
            const top10Views = sorted.slice(0, 10).reduce((sum, c) => sum + c.total_views, 0);
            const top10Share = (top10Views / totalViews * 100);

            // 注意：甜甜圈图已移除，改用水平柱状图展示各话题垄断度（见下方）
            // 保留整体市场数据用于洞察文本
            ...
            if (top3Share > 60) {
                level = '高度垄断';
                action = '建议寻找细分赛道切入，避免正面竞争。';
            } else if (top3Share > 40) {
                level = '中度集中';
                action = '头部有空间但需差异化定位。';
            } else {
                level = '较分散';
                action = '市场分散，新人友好，保持稳定更新即可。';
            }

            updateInsight('monopolyInsight',
                `市场格局：<span class="highlight">${level}</span>。Top 3频道占据<strong>${top3Share.toFixed(0)}%</strong>的总播放量，Top 10占<strong>${top10Share.toFixed(0)}%</strong>。`,
                action
            );
            ...
            if (videos && videos.length > 0) {
                ...
                Object.keys(topicKeywords).forEach(topic => {
                    topicStats[topic] = { videos: [], channels: {} };
                });
                ...
                videos.forEach(v => {
                    ...
                    for (const [topic, keywords] of Object.entries(topicKeywords)) {
                        if (keywords.some(kw => title.includes(kw.toLowerCase()))) {
                            topicStats[topic].videos.push(v);
                            const ch = v.channel_name || 'unknown';
                            if (!topicStats[topic].channels[ch]) {
                                topicStats[topic].channels[ch] = 0;
                            }
                            topicStats[topic].channels[ch] += v.view_count || 0;
                        }
                    }
                });
                ...
                for (const [topic, stats] of Object.entries(topicStats)) {
                    if (stats.videos.length < 5) continue; // 至少5个视频才有统计意义

                    const channelViews = Object.entries(stats.channels)
                        .map(([name, views]) => ({ name, views }))
                        .sort((a, b) => b.views - a.views);

                    const topicTotalViews = channelViews.reduce((sum, c) => sum + c.views, 0);
                    const top3Views = channelViews.slice(0, 3).reduce((sum, c) => sum + c.views, 0);
                    const top3Share = topicTotalViews > 0 ? (top3Views / topicTotalViews * 100) : 0;

                    let monopolyLevel, opportunity;
                    if (top3Share > 90) {
                        monopolyLevel = '极度垄断';
                        opportunity = '极难';
                    ...
                    topicMonopoly.push({
                        ...
                        channelCount: channelViews.length,
                        top3Share,
                        monopolyLevel,
                        opportunity
                    });
                ...
                // 按垄断度排序
                topicMonopoly.sort((a, b) => b.top3Share - a.top3Share);

                // 更新表格（同时更新两个位置的表格）
                const displayData = topicMonopoly.slice(0, 6);
                const tableIds = ['monopolyTableBody', 'topicMonopolyTableBody'];

                tableIds.forEach(tableId => {
                    const tableBody = document.getElementById(tableId);
                    if (tableBody && displayData.length > 0) {
                        tableBody.innerHTML = displayData.map((t, i) => {
                            const shareClass = t.top3Share > 80 ? 'highlight-value' : (t.top3Share < 50 ? 'high-value' : '');
                            const oppClass = t.opportunity === '极难' || t.opportunity === '很难' ? 'low-value' : (t.opportunity === '较易' ? 'high-value' : '');
                            return `<tr>
                                <td>${t.topic}</td>
                                <td class="${shareClass}">${t.top3Share.toFixed(1)}%</td>
                                <td>${t.monopolyLevel}</td>
                                <td class="${oppClass}">${t.opportunity}</td>
                            </tr>`;
                        }).join('');
                    } else if (tableBody) {
                        tableBody.innerHTML = '<tr><td colspan="4" style="text-align:center; color:#94a3b8;">暂无足够的话题数据</td></tr>';
                    }
                });

                // 更新水平柱状图（与表格数据对应，同时更新两个位置）
                const barChartIds = ['monopolyBarChart', 'globalMonopolyBarChart'];
                barChartIds.forEach(chartId => {
                    const barChart = document.getElementById(chartId);
                    if (barChart && displayData.length > 0) {
                        barChart.innerHTML = displayData.map(t => {
                            // 根据垄断度选择颜色
                            let barClass = '';
                            if (t.top3Share > 90) {
                                barClass = 'danger';  // 极度垄断 - 红色
                            } else if (t.top3Share > 80) {
                                barClass = 'warning'; // 高度垄断 - 橙色
                            } else if (t.top3Share > 60) {
                                barClass = '';        // 较垄断 - 默认蓝色
                            } else {
                                barClass = 'success'; // 分散 - 绿色
                            }
                            return `<div class="bar-item">
                                <span class="bar-label">${t.topic}</span>
                                <div class="bar-track">
                                    <div class="bar-fill ${barClass}" style="width: ${t.top3Share}%"></div>
                                </div>
                                <span class="bar-value">${t.top3Share.toFixed(1)}%</span>
                            </div>`;
                        }).join('');
                    } else if (barChart) {
                        barChart.innerHTML = '<div style="color: #94a3b8; text-align: center; padding: 20px;">暂无足够的话题数据</div>';
                    }
                });

                // 更新 tab3-sub2 结论区域
                const conclusion2 = document.getElementById('monopolyConclusion2');
                if (conclusion2 && displayData.length > 0) {
                    const avgShare = displayData.reduce((s, t) => s + t.top3Share, 0) / displayData.length;
                    const highMonopoly = displayData.filter(t => t.top3Share > 80);
                    const lowMonopoly = displayData.filter(t => t.top3Share < 50);
                    if (highMonopoly.length >= displayData.length * 0.6) {
                        conclusion2.innerHTML = `大部分话题 <strong>Top3频道占>${avgShare.toFixed(0)}%流量</strong>，头部效应明显。但<strong>${lowMonopoly.length}</strong>个话题（${lowMonopoly.map(t => t.topic).join('、')}）垄断度较低，新人可切入。`;
                    } else {
                        conclusion2.innerHTML = `话题垄断度分化：<strong>${highMonopoly.length}</strong>个高垄断话题（避开）vs <strong>${lowMonopoly.length}</strong>个低垄断话题（机会）`;
                    }
                }

                // 更新 tab3-sub2 行动建议
                const actions2 = document.getElementById('monopolyActions2');
                if (actions2 && displayData.length > 0) {
                    const bestOpportunity = displayData.filter(t => t.opportunity === '较易' || t.opportunity === '一般').slice(0, 2);
                    const avoid = displayData.filter(t => t.opportunity === '极难' || t.opportunity === '很难').slice(0, 1);
                    actions2.innerHTML = `
                        <div class="action-item">
                            <span class="action-icon">🔍</span>
                            <span>寻找垄断度<60%的细分话题</span>
                        </div>
                        ${bestOpportunity.length > 0 ? `<div class="action-item success">
                            <span class="action-icon">✅</span>
                            <span>推荐切入：${bestOpportunity.map(t => t.topic).join('、')}</span>
                        </div>` : ''}
                        ${avoid.length > 0 ? `<div class="action-item danger">
                            <span class="action-icon">⚠️</span>
                            <span>避免正面竞争：${avoid[0].topic}</span>
                        </div>` : ''}
                    `;
                }
                ...
                if (displayData.length > 0) {
                    const lowMonopolyTopics = displayData.filter(t => t.top3Share < 50);
                    const highMonopolyTopics = displayData.filter(t => t.top3Share > 80);
                    const lowTopicNames = lowMonopolyTopics.map(t => `"${t.topic}"`).join('、') || '暂无';
                    const avgShare = displayData.reduce((s, t) => s + t.top3Share, 0) / displayData.length;

                    registerPatternConclusion('tab1', '23', '话题垄断度',
                        '话题垄断度分析',
                        `大多数热门话题被Top3频道垄断${avgShare.toFixed(0)}%+流量。新进入者正面竞争几乎不可能。但${lowTopicNames}等细分话题垄断度较低（<50%），是新人切入的好机会。`
                    );

                    // Tab3 选题决策视角的模式23
                    registerPatternConclusion('tab3', '23', '话题垄断度',
                        '话题垄断度',
                        `选题建议：避开${highMonopolyTopics.length}个高垄断话题（Top3占>${avgShare.toFixed(0)}%），优先选择${lowTopicNames}等低垄断（<50%）领域。`
                    );
                }
            }
        }
        ...
        /**
         * 模式12：黑马频道发现
         * 找出快速增长的频道：从0涨到1000粉丝最快的
         */
        function renderPattern12(channels) {
            if (!channels || channels.length === 0) return;

            // 优先找一年内创建的新频道（真正的黑马）
            // 黑马定义：创建时间短（≤365天）+ 已达到1000粉 + 增长快
            const MAX_AGE_DAYS = 365; // 只看一年内的新频道
            const channelsWithAge = channels
                .filter(c => c.days_since_creation && c.days_since_creation > 0
                    && c.days_since_creation <= MAX_AGE_DAYS  // 限制频道年龄
                    && c.subscriber_count >= 1000)
                .map(c => ({
                    ...c,
                    subs_per_day: c.subs_per_day || (c.subscriber_count / c.days_since_creation),
                    efficiency: c.total_views / (c.subscriber_count || 1),
                }))
            ...
            const channelsWithRatio = channels
                ...
                .map(c => ({
                    ...c,
                    efficiency: c.total_views / c.subscriber_count,
                    avgViews: c.video_count > 0 ? c.total_views / c.video_count : 0
                }))
                .sort((a, b) => b.efficiency - a.efficiency);

            // 选择数据源：优先使用真实增长数据
            const useRealGrowth = channelsWithAge.length >= 3;
            const darkHorses = useRealGrowth ? channelsWithAge.slice(0, 10) : channelsWithRatio.slice(0, 10);

            if (darkHorses.length > 0) {
                // 使用气泡图展示
                const bubbleData = darkHorses.map(c => ({
                    x: c.subscriber_count,
                    y: c.total_views,
                    r: Math.min(30, Math.max(8, Math.sqrt(c.efficiency) * 2)),
                    label: c.channel_name
                }));

                renderBubble('darkHorseChart', bubbleData, {
                    xLabel: '订阅数',
                    yLabel: '总播放量',
                    xScale: 'logarithmic',
                    yScale: 'logarithmic'
                ...
                // 排行榜
                if (useRealGrowth) {
                    renderRankingBar('darkHorseRanking',
                        darkHorses.slice(0, 5).map(c => ({
                            label: (c.channel_name || '未知频道').substring(0, 12),
                            value: c.subs_per_day || 0
                        })),
                        { valueFormatter: v => `日均+${v.toFixed(0)}粉` }
                    );
                } else {
                    renderRankingBar('darkHorseRanking',
                        darkHorses.slice(0, 5).map(c => ({
                            label: (c.channel_name || '未知频道').substring(0, 15),
                            value: c.efficiency || 0
                        })),
                        { valueFormatter: v => `效率 ${v.toFixed(1)}x` }
                    );
                }

                // 更新洞察文案
                if (useRealGrowth) {
                    const fastest = darkHorses[0];
                    updateInsight('darkHorseInsight',
                        `发现<strong>${darkHorses.length}</strong>个快速增长频道。最快：<span class="highlight">${fastest.channel_name}</span>（${fastest.days_since_creation}天涨到${formatNumber(fastest.subscriber_count)}粉，日均<strong>+${fastest.subs_per_day.toFixed(0)}粉</strong>）。`,
                        `研究这些频道的冷启动策略，学习如何从0快速积累粉丝。`
                    );
                } else {
                    updateInsight('darkHorseInsight',
                        `发现<strong>${darkHorses.length}</strong>个黑马频道。最高效率：<span class="highlight">${darkHorses[0].channel_name}</span>（订阅${formatNumber(darkHorses[0].subscriber_count)}，播放${formatNumber(darkHorses[0].total_views)}，效率<strong>${darkHorses[0].efficiency.toFixed(1)}x</strong>）。`,
                        `重点研究这些高效频道的内容策略，他们可能掌握了爆款密码。`
                    );
                }

                // 渲染涨势线条图：真正的时间 vs 订阅增长
                const growthChart = document.getElementById('darkHorseGrowthChart');
                if (growthChart && useRealGrowth) {
                    // 使用真实时间序列数据
                    const topGrowers = darkHorses.slice(0, 3);

                    // 找出最长天数和最大订阅数用于归一化
                    const maxDays = Math.max(...topGrowers.map(c => c.days_since_creation), 365);
                    const maxSubs = Math.max(...topGrowers.map(c => c.subscriber_count));
                    ...
                    const colors = ['#22c55e', '#10b981', '#34d399']; // 绿色系

                    let svgLines = '';
                    let labels = '';
                    let dots = '';

                    // 每个频道：从(0,0)到(days, subs)画增长线
                    topGrowers.forEach((c, i) => {
                        const endX = padding.left + (c.days_since_creation / maxDays) * chartWidth;
                        const endY = padding.top + chartHeight - (c.subscriber_count / maxSubs) * chartHeight;
                        const startX = padding.left;
                        const startY = padding.top + chartHeight;

                        // 用二次贝塞尔曲线画增长曲线（起步慢，后期快）
                        const ctrlX = startX + (endX - startX) * 0.7;
                        const ctrlY = startY - (startY - endY) * 0.3;

                        svgLines += `<path d="M ${startX} ${startY} Q ${ctrlX} ${ctrlY} ${endX} ${endY}"
                            stroke="${colors[i]}" stroke-width="3" fill="none" stroke-linecap="round"
                            style="filter: drop-shadow(0 0 4px ${colors[i]}40);"/>`;

                        // 终点圆点
                        dots += `<circle cx="${endX}" cy="${endY}" r="5" fill="${colors[i]}"/>`;

                        // 频道名和数据标签
                        const channelName = c.channel_name || '未知频道';
                        const shortName = channelName.length > 10 ? channelName.substring(0, 10) + '..' : channelName;
                        labels += `<text x="${endX}" y="${Math.max(15, endY - 12)}" fill="${colors[i]}" font-size="10" text-anchor="middle" font-weight="bold">${shortName}</text>`;
                        labels += `<text x="${endX}" y="${endY + 16}" fill="${colors[i]}" font-size="9" text-anchor="middle">${c.days_since_creation || 0}天/${formatNumber(c.subscriber_count || 0)}粉</text>`;
                    });

                    // 坐标轴
                    const axes = `
                        <!-- X轴 -->
                        <line x1="${padding.left}" y1="${padding.top + chartHeight}" x2="${svgWidth - padding.right}" y2="${padding.top + chartHeight}" stroke="#475569" stroke-width="1"/>
                        <!-- Y轴 -->
                        <line x1="${padding.left}" y1="${padding.top}" x2="${padding.left}" y2="${padding.top + chartHeight}" stroke="#475569" stroke-width="1"/>
                        <!-- X轴标签 -->
                        <text x="${padding.left}" y="${svgHeight - 5}" fill="#64748b" font-size="9">0天</text>
                        <text x="${svgWidth - padding.right}" y="${svgHeight - 5}" fill="#64748b" font-size="9" text-anchor="end">${maxDays}天</text>
                        <text x="${(padding.left + svgWidth - padding.right) / 2}" y="${svgHeight - 5}" fill="#94a3b8" font-size="9" text-anchor="middle">频道年龄</text>
                        <!-- Y轴标签 -->
                        <text x="${padding.left - 5}" y="${padding.top + chartHeight}" fill="#64748b" font-size="9" text-anchor="end">0</text>
                        <text x="${padding.left - 5}" y="${padding.top + 5}" fill="#64748b" font-size="9" text-anchor="end">${formatNumber(maxSubs)}</text>
                        <text x="${padding.left - 5}" y="${(padding.top + padding.top + chartHeight) / 2}" fill="#94a3b8" font-size="9" text-anchor="end" transform="rotate(-90, ${padding.left - 30}, ${(padding.top + padding.top + chartHeight) / 2})">订阅数</text>
                    `;

                    // 背景网格
                    const grid = `
                        <line x1="${padding.left}" y1="${padding.top + chartHeight * 0.5}" x2="${svgWidth - padding.right}" y2="${padding.top + chartHeight * 0.5}" stroke="#334155" stroke-width="0.5" stroke-dasharray="3,3"/>
                        <line x1="${padding.left}" y1="${padding.top + chartHeight * 0.25}" x2="${svgWidth - padding.right}" y2="${padding.top + chartHeight * 0.25}" stroke="#334155" stroke-width="0.5" stroke-dasharray="3,3"/>
                    `;
                    ...
                    // 回退：使用效率对比图
                    const topDarkHorses = darkHorses.slice(0, 3);
                    const midIndex = Math.floor(channelsWithRatio.length / 2);
                    const medianChannel = channelsWithRatio[midIndex];
                    const maxEfficiency = topDarkHorses[0]?.efficiency || 1;
                    ...
                    topDarkHorses.forEach((c, i) => {
                        const slope = Math.min(0.95, c.efficiency / maxEfficiency);
                        const endY = svgHeight * (1 - slope);
                        const startX = 20 + i * 12;
                        const endX = 380 - i * 20;
                        const controlX = (startX + endX) / 2;
                        const controlY = endY + (svgHeight - endY) * 0.3;
                        svgLines += `<path d="M ${startX} ${svgHeight} Q ${controlX} ${controlY} ${endX} ${endY}"
                            stroke="${colors[i]}" stroke-width="3" fill="none" stroke-linecap="round"/>`;
                        const shortName = c.channel_name.length > 8 ? c.channel_name.substring(0, 8) + '..' : c.channel_name;
                        labels += `<text x="${endX}" y="${Math.max(15, endY - 8)}" fill="${colors[i]}" font-size="11" text-anchor="middle" font-weight="bold">${shortName}</text>`;
                        labels += `<text x="${endX}" y="${endY + 16}" fill="${colors[i]}" font-size="10" text-anchor="middle">${Math.round(c.efficiency)}×</text>`;
                        svgLines += `<circle cx="${endX}" cy="${endY}" r="5" fill="${colors[i]}"/>`;
                    });

                    if (medianChannel) {
                        const slope = Math.min(0.95, medianChannel.efficiency / maxEfficiency);
                        const endY = svgHeight * (1 - slope);
                        svgLines += `<path d="M 20 ${svgHeight} Q 200 ${svgHeight - 15} 360 ${Math.min(svgHeight - 5, endY)}"
                            stroke="#64748b" stroke-width="2" fill="none" stroke-dasharray="5,3"/>`;
                        svgLines += `<circle cx="360" cy="${Math.min(svgHeight - 5, endY)}" r="4" fill="#64748b"/>`;
                        labels += `<text x="360" y="${Math.min(svgHeight - 5, endY) + 16}" fill="#94a3b8" font-size="9" text-anchor="middle">中位数 ${medianChannel.efficiency.toFixed(1)}×</text>`;
                    }

                    growthChart.innerHTML = `
                        <svg viewBox="0 0 ${svgWidth} ${svgHeight + 25}" preserveAspectRatio="xMidYMid meet" style="width: 100%; height: 100%;">
                            <line x1="0" y1="${svgHeight}" x2="${svgWidth}" y2="${svgHeight}" stroke="#334155" stroke-width="1"/>
                            ${svgLines}
                            ${labels}
                            <text x="10" y="${svgHeight + 15}" fill="#64748b" font-size="9">起点</text>
                            <text x="390" y="${svgHeight + 15}" fill="#64748b" font-size="9" text-anchor="end">现在</text>
                            <text x="10" y="12" fill="#64748b" font-size="9">爆发力 ↑</text>
                        </svg>
                    `;
                }

                // 注册模式结论到信息报告
                const top = darkHorses[0];
                const topEfficiency = top.efficiency ? top.efficiency.toFixed(0) : '500';
                registerPatternConclusion('tab1', '12', '黑马频道',
                    '黑马频道排行',
                    `订阅1-5万的频道最容易出爆款，爆发倍率可达${topEfficiency}倍以上。大V反而难出爆款（粉丝增长→爆发倍率下降）。新频道有逆袭机会！`
                );
            }
        }
         ...
         * 使用雷达图展示头部频道特征
         */
        function renderPattern31(channels) {
            if (!channels || channels.length === 0) return;

            const sorted = [...channels].sort((a, b) => b.total_views - a.total_views);
            const topChannels = sorted.slice(0, 10);
            const allAvg = {
                videoCount: channels.reduce((s, c) => s + c.video_count, 0) / channels.length,
                avgViews: channels.reduce((s, c) => s + (c.video_count > 0 ? c.total_views / c.video_count : 0), 0) / channels.length,
                subscribers: channels.reduce((s, c) => s + (c.subscriber_count || 0), 0) / channels.length
            };

            const topAvg = {
                videoCount: topChannels.reduce((s, c) => s + c.video_count, 0) / topChannels.length,
                avgViews: topChannels.reduce((s, c) => s + (c.video_count > 0 ? c.total_views / c.video_count : 0), 0) / topChannels.length,
                subscribers: topChannels.reduce((s, c) => s + (c.subscriber_count || 0), 0) / topChannels.length
            };

            // 归一化分数（相对于全体平均）
            const labels = ['视频数量', '平均播放', '订阅规模', '内容效率', '活跃度'];
            const topScores = [
                Math.min(100, (topAvg.videoCount / allAvg.videoCount) * 50),
                Math.min(100, (topAvg.avgViews / allAvg.avgViews) * 50),
                Math.min(100, (topAvg.subscribers / allAvg.subscribers) * 50),
                80, // placeholder
                70  // placeholder
            ];
            const avgScores = [50, 50, 50, 50, 50];

            renderRadar('expertProfileChart', labels, [
                { label: 'Top 10 频道', data: topScores, color: 'rgba(6, 182, 212, 0.8)' },
                { label: '平均水平', data: avgScores, color: 'rgba(148, 163, 184, 0.5)' }
            ]);

            updateInsight('expertProfileInsight',
                `Top 10频道特征：平均发布<strong>${Math.round(topAvg.videoCount)}</strong>个视频，均播<strong>${formatNumber(Math.round(topAvg.avgViews))}</strong>，订阅<strong>${formatNumber(Math.round(topAvg.subscribers))}</strong>。`,
                `高手频道的核心特征：持续产出+高质量内容。`
            );

            // 注册模式结论到信息报告
            const topOne = topChannels[0];
            if (topOne) {
                const topAvgViews = (topOne.total_views / topOne.video_count / 10000).toFixed(1);
                registerPatternConclusion('tab1', '31', '高手画像',
                    'Top5高手频道分析',
                    `高手的共同特征：垂直深耕单一领域 + 稳定更新 + 标题含关键词 + 时长10-30分钟。${topOne.channel_name}以${topOne.video_count}条视频获得${formatNumber(topOne.total_views)}播放，均播${topAvgViews}万，是效率最高的频道。`
                );
            }
        }
        ...
        function renderDarkHorseRanking(darkHorse) {
            ...
            if (conclusionEl && darkHorse.channels.length > 0) {
                ...
                registerPatternConclusion('tab6', '12', '黑马频道特征',
                    `${darkHorse.channels.length}个黑马频道分析`,
        ...
        function renderRegionDistribution(data) {
            ...
            if (data.regions && data.regions.length >= 2) {
                ...
                registerPatternConclusion('tab2', '20', '地区差异与跨境机会',
                    `${data.regions.length}个地区播放量对比`,
                    `${top.region}地区均播最高（${topAvgText}），是${bottom.region}地区的${ratio}倍。存在明显的地域套利空间，建议针对${top.region}地区优化内容策略。`
                );

                // 模式21: 多语言市场对比（静态数据）
                registerPatternConclusion('tab2', '21', '多语言市场对比',
                    '英语/日语市场对比',
                    '气功英语市场均播是日语的8倍，太极英语是日语的2.9倍。建议英语内容优先，可用 Qigong、Tai Chi 关键词进入英语市场。'
                );

                // 模式24: 国家×内容收益矩阵（基于前端静态数据）
                registerPatternConclusion('tab2', '24', '国家内容收益综合分析',
                    '各国CPM×爆款率矩阵',
                    '第一优先：Tai Chi教程@美国（CPM$12 + 40%爆款率）；第二优先：穴位按摩@新加坡/香港（CPM$17+市场空白）；避免：食疗@台湾（红海219条+低CPM$3.5）。同样10万播放，美国$1,200 vs 台湾$350。'
                );
            }
        }
        ...
        function updateChannelPatterns(channels, insights) {
            ...
            // 更新高手画像表格（模式31）- 按总播放排序的头部频道
            const topChannelsTbody = document.getElementById('topChannelTableBody');
            if (topChannelsTbody && topChannels.length > 0) {
                topChannelsTbody.innerHTML = topChannels.map((c, i) => {
                    const subText = c.subscriber_count >= 10000
                        ? (c.subscriber_count / 10000).toFixed(2) + '万'
                        : (c.subscriber_count || '-');
                    const totalViewText = c.total_views >= 10000
                        ? (c.total_views / 10000).toFixed(0) + '万'
                        : c.total_views;
                    const avgViewText = c.avg_views >= 10000
                        ? (c.avg_views / 10000).toFixed(1) + '万'
                        : c.avg_views;
                    const viewClass = i === 0 ? 'highlight-value' : '';
                    const channelLink = c.channel_id
                        ? `<a href="https://www.youtube.com/channel/${c.channel_id}" target="_blank" rel="noopener" class="channel-link">${c.channel_name || '未知'}</a>`
                    ...
                    return `<tr>
                        ...
                        <td>${subText}</td>
                        <td>${c.video_count || '-'}</td>
                        <td class="${viewClass}">${totalViewText}</td>
                        <td>${avgViewText}</td>
                    </tr>`;
                }).join('');
            }
            ...
            // 更新高手画像样本量
            const topSampleEl = document.getElementById('topChannelSample');
            if (topSampleEl) {
                topSampleEl.textContent = `N = ${channels.length}`;
            }
                ...
                content: `
                    <div class="pattern-flow">
                        <div class="pattern-step expanded">
                            <div class="pattern-step-header" onclick="togglePatternStep(this)">
                                <span class="pattern-step-number">1</span>
                                <span class="pattern-step-title">原始数据 - Top5 高手排行</span>
                                <span class="pattern-step-toggle">▼</span>
                            </div>
                            <div class="pattern-step-content">
                                <div class="pattern-step-inner">
                                    <table class="pattern-data-table">
                                        ...
                                        <tbody>
                                            <tr class="highlight"><td>🥇</td><td>laxnetcm</td><td>3</td><td class="highlight-value">27.3万</td><td class="highlight-value">51.55</td><td>效率王</td></tr>
                                    ...
                                    <p style="color: var(--color-text-muted); font-size: 0.85em; margin-top: 12px;">效率 = 均播放 / 订阅数（每个订阅带来多少播放）</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="pattern-step expanded">
                            <div class="pattern-step-header" onclick="togglePatternStep(this)">
                                <span class="pattern-step-number">2</span>
                                <span class="pattern-step-title">高手特征雷达图</span>
                                <span class="pattern-step-toggle">▼</span>
                            </div>
                            <div class="pattern-step-content">
                                <div class="pattern-step-inner">
                                    <div style="display: flex; gap: 24px; align-items: center; flex-wrap: wrap;">
                                        <!-- 雷达图 -->
                                        <div class="radar-chart-container" style="flex: 1; min-width: 220px;">
                                            <div class="radar-chart" style="width: 220px; height: 220px;">
                                                <svg viewBox="0 0 200 200">
                                                    <!-- 背景网格 -->
                                                    <polygon class="radar-grid" points="100,20 170,60 170,140 100,180 30,140 30,60"/>
                                                    <polygon class="radar-grid" points="100,40 150,70 150,130 100,160 50,130 50,70"/>
                                                    ...
                                                    <line class="radar-axis" x1="100" y1="100" x2="170" y2="60"/>
                                                    <line class="radar-axis" x1="100" y1="100" x2="170" y2="140"/>
                                                    <line class="radar-axis" x1="100" y1="100" x2="100" y2="180"/>
                                                    <line class="radar-axis" x1="100" y1="100" x2="30" y2="140"/>
                                                    <line class="radar-axis" x1="100" y1="100" x2="30" y2="60"/>
                                                    <!-- 高手数据区域 (效率95%, 爆款率90%, 稳定性85%, 内容深度80%, 互动率70%, 更新频率60%) -->
                                                    <polygon class="radar-area" points="100,24 166,62 162,135 100,172 38,130 40,65"/>
                                                    <!-- 数据点 -->
                                                    <circle class="radar-point" cx="100" cy="24" r="4"/>
                                                    <circle class="radar-point" cx="166" cy="62" r="4"/>
                                                    <circle class="radar-point" cx="162" cy="135" r="4"/>
                                                    <circle class="radar-point" cx="100" cy="172" r="4"/>
                                                    <circle class="radar-point" cx="38" cy="130" r="4"/>
                                                    <circle class="radar-point" cx="40" cy="65" r="4"/>
                                                    <!-- 标签 -->
                                                    <text class="radar-label" x="100" y="12">效率 95%</text>
                                                    <text class="radar-label" x="180" y="60">爆款率 90%</text>
                                                    <text class="radar-label" x="180" y="145">稳定性 85%</text>
                                                    <text class="radar-label" x="100" y="195">内容深度 80%</text>
                                                    <text class="radar-label" x="15" y="145">互动质量 70%</text>
                                                    <text class="radar-label" x="15" y="60">更新频率 60%</text>
                                                </svg>
                                            </div>
                                        </div>
                                        <!-- 对比表格 -->
                                        <div style="flex: 1; min-width: 240px;">
                                            <table class="pattern-data-table">
                                                ...
                                                <tbody>
                                                    <tr><td>效率</td><td class="highlight-value">>20</td><td>&lt;5</td><td class="highlight-value">4×</td></tr>
                                                    <tr><td>爆款率</td><td class="highlight-value">>66%</td><td>&lt;30%</td><td class="highlight-value">2×</td></tr>
                                                    <tr><td>稳定性</td><td>均高</td><td>波动大</td><td>稳定</td></tr>
                                                </tbody>
                                            </table>
                                            <p style="color: var(--color-accent); font-size: 0.85em; margin-top: 12px; text-align: center;">高手的能力雷达图呈现全面发展特征</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="pattern-step expanded">
                            <div class="pattern-step-header" onclick="togglePatternStep(this)">
                                <span class="pattern-step-number">3</span>
                                <span class="pattern-step-title">高手内容策略案例</span>
                                <span class="pattern-step-toggle">▼</span>
                            </div>
                            <div class="pattern-step-content">
                                <div class="pattern-step-inner">
                                    <div style="display: flex; flex-direction: column; gap: 12px;">
                                        <div style="padding: 14px; background: var(--color-bg-secondary); border-radius: 8px; border-left: 3px solid var(--color-accent);">
                                            <div style="font-weight: 600; color: var(--color-accent); margin-bottom: 6px;">🥇 laxnetcm（效率王，5300订阅→均播27万）</div>
                                            <div style="font-size: 0.9em; color: var(--color-text-secondary);">内容：腰腿关节痛、补肾<br>标题模式：痛点+解决方案+低成本("不到2块钱")</div>
                                        </div>
                                        <div style="padding: 14px; background: var(--color-bg-secondary); border-radius: 8px; border-left: 3px solid var(--color-confidence-high);">
                                            <div style="font-weight: 600; color: var(--color-confidence-high); margin-bottom: 6px;">🥈 养生老郑（162万爆款）</div>
                                            <div style="font-size: 0.9em; color: var(--color-text-secondary);">内容：日常食材养生（生姜、香菜、桑叶）<br>核心公式：普通食材 × 意想不到的功效 = 爆款</div>
                                        </div>
                                        <div style="padding: 14px; background: var(--color-bg-secondary); border-radius: 8px; border-left: 3px solid var(--color-confidence-medium);">
                                            <div style="font-weight: 600; color: var(--color-confidence-medium); margin-bottom: 6px;">🥉 太极芸庄（100%爆款率）</div>
                                            <div style="font-size: 0.9em; color: var(--color-text-secondary);">内容：八段锦、站桩导引<br>核心公式：传统功法 × 完整教学 × 陪伴感 = 爆款</div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="pattern-step expanded">
                            <div class="pattern-step-header" onclick="togglePatternStep(this)">
                                <span class="pattern-step-number">4</span>
                                <span class="pattern-step-title">结论与行动</span>
                                <span class="pattern-step-toggle">▼</span>
                            </div>
                            <div class="pattern-step-content">
                                <div class="pattern-step-inner">
                                    <div class="pattern-conclusion">
                                        ...
                                        <div class="pattern-conclusion-text">
                                            高手的共同特征：<strong>效率 >20</strong>（每订阅带来20+播放）、<strong>爆款率 >66%</strong>、<strong>内容质量稳定</strong>。可复制的成功公式已明确。
                                        </div>
                                    </div>
                                    <div class="pattern-actions">
                                        <div class="pattern-action">
        ...
        // ========== 用户洞察图表（模式38-42）- 从 API 获取真实数据 ==========

        // 用户洞察数据缓存
        let userInsightData = null;

        // 初始化用户洞察 - 从 API 获取真实数据
        async function initUserInsightCharts() {
            console.log('正在从 API 加载用户洞察数据...');

            try {
                const response = await fetch(`${API_BASE}/api/user-insights/${encodeURIComponent(currentKeyword)}`);
                const result = await response.json();

                if (result.status === 'ok' && result.total_comments > 0) {
                    userInsightData = result;
                    console.log('✓ 用户洞察数据加载成功:', result.total_comments, '条评论');
                    renderUserInsightData(result);

                    // 注册用户洞察结论
                    // 重新渲染信息报告以包含用户洞察结论（模式结论已在 renderUserInsightData 注册）
                    renderInfoReportFromConclusions();
                } else {
                    console.warn('⚠️ 暂无评论数据:', result.message || '');
                    showNoCommentData();
                }
            } catch (error) {
        ...
        function renderUserInsightData(data) {
            ...
            // 模式40: 情感分布
            if (data.sentiment) {
                const total = data.sentiment.positive.count + data.sentiment.neutral.count + data.sentiment.negative.count;
            ...
            console.log('✓ 用户洞察模式结论注册完成，共', tabConclusions.tab8.items.length, '条');
        }

        // 注册静态用户洞察结论（基于截图数据，作为 fallback）
        function registerStaticUserInsightConclusions() {
            // 如果已有动态数据注册的结论，则跳过
            if (tabConclusions.tab8.items.length > 0) {
                console.log('用户洞察已有动态结论，跳过静态注册');
                return;
            }

            console.log('注册静态用户洞察结论...');

            // 模式38: 评论热词（基于截图数据）
            registerPatternConclusion('tab8', '38', '评论热词',
                '评论热词',
                '高频词：感恩(1,406)、分享(1,319)、謝謝(1,314)。感谢类词汇占主导，说明用户满意度极高。"老师/医师"高频出现表明用户视创作者为专家权威。'
            );
        ...
        function renderQuestionsChart(questions) {
            ...
            chartInstances['questionsChart'] = chart;

            // 更新洞察文字
            const insight = document.getElementById('questionsInsight');
            if (insight) {
                const top = sortedTypes[0];
                insight.querySelector('.chart-insight-content').innerHTML = `
        ...
        function renderHighLikedStats(stats) {
            ...
            if (cards.length >= 4) {
                cards[0].querySelector('div:first-child').textContent = `${stats.avg_length}字`;
                cards[1].querySelector('div:first-child').textContent = `${stats.has_experience_pct}%`;
                cards[2].querySelector('div:first-child').textContent = `${stats.has_question_pct}%`;
                cards[3].querySelector('div:first-child').textContent = stats.max_likes.toLocaleString();
            }
        }
        ...
        // 全局认知洞察（模式结论已在 renderPattern4/renderPattern23 等函数注册，这里只返回文本）
        ...
        // 套利挖掘洞察（模式结论已在 renderRegionDistribution 注册，这里只返回文本）
        ...
        // 返回首页
        function goBack() {
            // 如果在 Vercel 部署环境，刷新当前页面（因为 insight-system 就是首页）
            // 如果在本地开发环境，返回 demo.html
            if (window.location.hostname.includes('vercel.app')) {
                window.location.href = '/';
            } else {
                window.location.href = '/';  // API 服务器根路径返回 demo.html
            }
        }
        ...
        function renderOverview(data) {
            ...
            if (data.channels) {
                ...
                if (topCountryEl && countryCount > 0) {
                    ...
                    // 缩短国家名
                    const shortName = {'Hong Kong': 'HK', 'Taiwan': 'TW', 'United States': 'US', 'Singapore': 'SG'}[top[0]] || top[0];
            ...
            if (!scatterCanvas || !data.videos || data.videos.length === 0) return;
            ...
            // 按月聚合话题播放量
            const monthTopicData = {}; // { "2025-06": { "八段锦": {views, count}, ... } }

            data.videos.forEach(v => {
                if (!v.published_at) return;
                const monthKey = v.published_at.slice(0, 7); // "YYYY-MM"

                // 识别视频话题：优先用 tags，否则用标题
                let topics = [];
                if (v.tags && Array.isArray(v.tags)) {
                ...
                if (!monthTopicData[monthKey]) monthTopicData[monthKey] = {};
                topics.forEach(topic => {
                    if (!monthTopicData[monthKey][topic]) {
                        monthTopicData[monthKey][topic] = { views: 0, count: 0 };
                    }
                    monthTopicData[monthKey][topic].views += v.view_count || 0;
                    monthTopicData[monthKey][topic].count += 1;
                });
            });

            // 构建散点数据（只取最近12个月）
            const months = Object.keys(monthTopicData).sort().slice(-12);
            const datasets = [];

            console.log('领域概览 - Top话题:', topTopics);
            console.log('领域概览 - 月份:', months);

            topTopics.forEach(topic => {
                const points = [];
                months.forEach((m, idx) => {
                    const topicData = monthTopicData[m]?.[topic];
                    if (topicData && topicData.count > 0) {
                        points.push({
                            x: idx,
                            y: topicData.views,
                            r: Math.min(Math.sqrt(topicData.count) * 2 + 3, 18),
                            month: m,
                            topic: topic,
            ...
            console.log('领域概览 - 数据集:', datasets.length, '个话题');
                        ...
                        tooltip: {
                            callbacks: {
                                label: function(ctx) {
                                    ...
                                    return [
                                        `${p.topic} (${p.month})`,
                                        `播放量: ${formatNumber(p.y)}`,
                                        `视频数: ${p.count}`,
                                        `均播: ${formatNumber(p.avgViews)}`
                                    ];
                                }
                            }
                        }
                        ...
                        x: {
                            ...
                            ticks: {
                                ...
                                callback: function(value) {
                                    ...
                                    return m ? m.slice(2).replace('-', '/') : ''; // "25/06" 格式
                                }
                            },
                            title: { display: false }
                        },
